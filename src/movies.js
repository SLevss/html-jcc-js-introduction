...
const ulElementPop = document.querySelector(".movies-popul");
const ulElementNotPop = document.querySelector(".movies-not-popul");
ulElement.innerHTML = getPosterImages();
ulElementPop.innerHTML = getPopularImages();
ulElementNotPop.innerHTML = getNotPopularImages();
...
function getPopularImages() {
    const ulElementPop = moviesData.results.reduce((maxPopMov, currentMov) => {
        return (maxPopMov.popularity > currentMov.popularity) ? maxPopMov : currentMov
    });
    const moviePop = 
...
function getNotPopularImages() {
    const ulElementNotPop = moviesData.results.reduce((minPopMov, currentMov) => {
        return (minPopMov.popularity > currentMov.popularity) ? currentMov : minPopMov
    });
    const moviePop =
